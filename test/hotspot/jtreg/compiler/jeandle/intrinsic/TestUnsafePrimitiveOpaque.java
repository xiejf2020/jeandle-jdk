 ...
 * @test
 * @key randomness
 * @summary Test naturally aligned Jeandle Unsafe primitive opaque get/put intrinsics
 ...
 * @run main/othervm compiler.jeandle.intrinsic.TestUnsafePrimitiveOpaque
 */
...
public class TestUnsafePrimitiveOpaque {
    private static final String RAW_CLASS =
            "compiler.jeandle.intrinsic.RawUnsafePrimitiveOpaque";
    private static final String[] IDS = {
            "_getBooleanOpaque", "_getByteOpaque", "_getShortOpaque",
            "_getCharOpaque", "_getIntOpaque", "_getLongOpaque", "_getFloatOpaque", "_getDoubleOpaque",
            "_putBooleanOpaque", "_putByteOpaque", "_putShortOpaque",
            "_putCharOpaque", "_putIntOpaque", "_putLongOpaque", "_putFloatOpaque", "_putDoubleOpaque"
    };

    public static void main(String[] args) throws Exception {
        String enabledDump = Files.createTempDirectory("jeandle_unsafe_opaque_on").toString();
        ...
        checkDynamicAddressIR(enabledDump);

        String sigbusDump = Files.createTempDirectory("jeandle_unsafe_opaque_sigbus").toString();
        ...
        Asserts.assertTrue(Pattern.compile("(?m)Unsafe(?:\\.|::)getByteOpaque"
                + ".*is parsed as intrinsic").matcher(sigbus.getOutput()).find(),
                "SIGBUS path did not compile getByteOpaque as an intrinsic");

        String disabledDump = Files.createTempDirectory("jeandle_unsafe_opaque_control_off").toString();
        ...
        String inlineUnsafeDisabledDump = Files.createTempDirectory("jeandle_unsafe_opaque_inline_unsafe_off").toString();
    ...
    private static OutputAnalyzer runChild(String dumpPath, boolean enabled, boolean inlineUnsafeOff,
        ...
        ArrayList<String> args = new ArrayList<>(List.of(
                ...
                "-XX:CompileCommand=dontinline," + TestWrapper.class.getName() + "::getByteDynamic"));
    ...
    private static void checkEnabledIR(String dumpPath) throws Exception {
        checker(dumpPath, "getBoolean", Object.class, long.class)
                .checkPattern("load atomic i8, ptr addrspace\\(1\\).*monotonic");
        checker(dumpPath, "getBoolean", Object.class, long.class)
        ...
        checker(dumpPath, "getShort", Object.class, long.class)
                .checkPattern("load atomic i16, ptr addrspace\\(1\\).*monotonic");
        checker(dumpPath, "getChar", Object.class, long.class)
                .checkPattern("load atomic i16, ptr addrspace\\(1\\).*monotonic");
        checker(dumpPath, "getInt", Object.class, long.class)
                .checkPattern("load atomic i32, ptr addrspace\\(1\\).*monotonic");
        checker(dumpPath, "getLong", Object.class, long.class)
                .checkPattern("load atomic i64, ptr addrspace\\(1\\).*monotonic");
        ...
        for (String name : RawClass.PUT_METHODS) {
            ...
            rawChecker.checkPattern("store atomic i(?:8|16) .*monotonic");
        }
    }

    private static void checkZeroAddressIR(String dumpPath) throws Exception {
        FileCheck checker = new FileCheck(dumpPath,
                TestWrapper.class.getMethod("getByteMaybeZero", Holder.class), false);
        // The compiled-but-unexecuted null+0 path must retain normal Unsafe
        // fallback; emitting inttoptr(0) would be undefined LLVM IR.
        checker.checkNotPattern("inttoptr i64 0 to ptr");
    }

    private static void checkDynamicAddressIR(String dumpPath) throws Exception {
        FileCheck checker = new FileCheck(dumpPath,
                TestWrapper.class.getMethod("getByteDynamic", Object.class, long.class), false);
        checker.checkPattern("unsafe_raw_zero_address");
        checker.checkPattern("load atomic i8, ptr.*monotonic");
        checker.checkPattern("deopt");
    }

    private static void checkDisabledIR(String dumpPath) throws Exception {
        String[] names = {"getBoolean", "getByte", "getShort", "getChar", "getInt", "getLong", "getFloat", "getDouble"};
        for (String name : names) {
            FileCheck check = checker(dumpPath, name, Object.class, long.class);
            check.checkNot("unsafe_opaque_addr");
        }
        checker(dumpPath, "putBoolean", Object.class, long.class, boolean.class)
                .checkNot("unsafe_opaque_addr");
        checker(dumpPath, "putByte", Object.class, long.class, byte.class)
                .checkNot("unsafe_opaque_addr");
        checker(dumpPath, "putShort", Object.class, long.class, short.class)
                .checkNot("unsafe_opaque_addr");
    ...
    public static class TestWrapper {
        ...
        public static void main(String[] args) throws Exception {
            ...
            Asserts.assertEquals((byte) 0, getByteMaybeZero(new Holder()));
        ...
        private static void testHeapValues() throws Exception {
            ...
            for (int i = 0; i < 1_000; i++) {
                ...
                Asserts.assertEquals(integer, getInt(h, io));
                Asserts.assertEquals(longer, getLong(h, lo));
                ...
                Asserts.assertEquals(~integer, h.i);
                Asserts.assertEquals(~longer, h.l);
        ...
        private static void testNativeValues() {
            ...
            try {
                ...
                for (int i = 0; i < 20_000; i++) {
                    Asserts.assertEquals((byte) 0x81, getByteDynamic(null, address));
                }
                ...
                putInt(null, address, 0x87654321);
                Asserts.assertEquals(0x87654321, U.getInt(address));
                Asserts.assertEquals(0x87654321, getInt(null, address));
                putLong(null, address, 0x8877665544332211L);
                Asserts.assertEquals(0x8877665544332211L, U.getLong(address));
                Asserts.assertEquals(0x8877665544332211L, getLong(null, address));
        ...
        private static void testSigbus() throws Exception {
            java.nio.file.Path path = Files.createTempFile("jeandle-unsafe-opaque-", ".bin");
            try {
    ...
    static class RawClass implements Opcodes {
        ...
        static byte[] generate() {
            ...
            emitPut(cw, "putBooleanRaw", "putBooleanOpaque", "Z");
            emitPut(cw, "putByteRaw", "putByteOpaque", "B");
            emitPut(cw, "putShortRaw", "putShortOpaque", "S");
            emitPut(cw, "putCharRaw", "putCharOpaque", "C");
            cw.visitEnd();
